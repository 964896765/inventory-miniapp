...
export default function DocsListScreen() {
  ...
  const params = useLocalSearchParams<{ type?: string; range?: string; mine?: string }>();
  const [items, setItems] = useState<any[]>([]);
  const [refreshing, setRefreshing] = useState(false);

  const load = useCallback(async () => {
    const res = await api.docs.list({ type: params.type, range: params.range, mine: params.mine });
    setItems(res.items || []);
  }, [params.type, params.range, params.mine]);
  ...
  return (
    <ScreenContainer className="bg-background">
      <View className="px-4 pt-4">
        <Text className="text-2xl font-bold text-foreground">单据列表</Text>
        <Text className="text-sm text-muted mt-1">{params.type ? `类型：${params.type}` : "全部类型"}</Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={<View className="items-center py-10"><Text className="text-sm text-muted">暂无单据</Text></View>}
      />
    </ScreenContainer>
  );
}
